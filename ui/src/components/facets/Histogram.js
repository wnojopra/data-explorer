...
class Histogram extends Component {
  ...
  onClick(event, item) {
    ...
    if (item && item.datum) {
      ...
      const facetValue =
        ("facet_value" in item.datum ? item.datum.facet_value : item.datum.value) === 'True' ? 'true' :
        ("facet_value" in item.datum ? item.datum.facet_value : item.datum.value) === 'False' ? 'false':
